  ...
  if [ -e $CARDSDIR/${name}_extramodels.dat ]; then
    ...
    do
      ...
      if [[ $model = *[!\ ]* ]]; then
        ...
        #wget --no-verbose --no-check-certificate https://github.com/casfisica/VLF_SignalGenerator/blob/master/Model/$model
	cp /afs/cern.ch/user/c/csalazar/WorkPublic/VLF_SignalGenerator/Model/$model ./
        cd models
        if [[ $model == *".zip"* ]]; then
          unzip ../$model
        elif [[ $model == *".tgz"* ]]; then
